...
def generate_video_metadata(absolute_paths, relative_paths, out_file):
    ...
    with tqdm(list(zip(absolute_paths, relative_paths))) as pbar:
        for i, (absolute_path, relative_path) in enumerate(pbar):
            ...
            vids.append(meta)
            pbar.set_description(f"{len(vids)}/{i+1}/{len(pbar)}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--basedir', help='Base directory containing videos', default="/share/pi/cleemess/eeg-summaries/good_video_lpch")
    # parser.add_argument('--file_list', help='List of video files', default="/share/pi/cleemess/file-conversion-pipeline/all_mp4s.txt")
    # parser.add_argument('--file_list', help='List of video files', default="/home/dwliang/mp4s.txt")
    parser.add_argument('--file_list', help='List of video files', default="/share/pi/cleemess/eeg-summaries/mp4s.txt")
    ...
    # parser.add_argument('--outfile', help='Output json file containing metadata for all videos', default="/home/dwliang/workshop-metadata.json")
    parser.add_argument('--outfile', help='Output json file containing metadata for all videos', default="/share/pi/cleemess/eeg-summaries/metadata.json")
    ...
    meta = generate_video_metadata(absolute_paths, relative_paths, outfile)
